...
map = (0.. W*H-1).to_a
count = 0
map.combination(W * H / 2){|blue|             # 把一半標為藍色
  if blue.include?(0) then                    # 左上角固定為藍色
    white = map - blue                        # 剩下的是白色
    check(blue, blue[0])                      # 藍色是否互相連接
    check(white, white[0]) if blue.size == 0  # 白色是否互相連接
    count += 1 if white.size == 0         # 如果兩種顏色都符合條件則計入結果
  end
}
puts count
